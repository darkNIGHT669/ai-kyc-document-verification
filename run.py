...
def process_dataset(dataset_dir, output_file, ocr_provider, llm_model, preprocess):
    ...
    # Debug: Ensure each document result is logged
    if not results:
        logger.warning("⚠️ No results were generated. Check OCR and LLM outputs!")
    else:
        for idx, res in enumerate(results):
            person_id = res.get('person_id', f'Person_{idx+1}')
            status = res.get('overall_status', 'UNKNOWN')
            logger.info(f"[{idx+1}] {person_id}: {status}")
        logger.info(f"✅ {len(results)} results generated.")
    
    # Explicitly save JSON results
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4, ensure_ascii=False)
    logger.info(f"📄 verification_results.json saved at {output_file}")
...
def generate_report(results, output_file='report.txt'):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for result in results:
            f.write("-" * 70 + "\n")
            f.write(f"Person ID: {result.get('person_id','N/A')}\n")
            f.write(f"Overall Status: {result.get('overall_status','UNKNOWN')}\n")
            f.write("-" * 70 + "\n\n")
            
            f.write("Extracted Data:\n")
            for doc_name, doc_data in result.get('extracted_data', {}).items():
                f.write(f"\n  {doc_name}:\n")
            ...
            for rule, details in result.get('verification_results', {}).items():
